    ...
    return (
      <div className="bg-white dark:bg-slate-900 p-4 md:p-6 rounded-[24px] md:rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-sm">
        ...
        <div className="grid grid-cols-7 gap-1">
          {['D','S','T','Q','Q','S','S'].map(d => (
            <div key={d} className="text-center text-[7px] md:text-[8px] font-black text-slate-400 py-1 uppercase tracking-tighter">{d}</div>
          ))}
          {days.map((d, i) => {
              ...
              {data.reminders.length > 0 ? (
                <div className="space-y-2">
                  {data.reminders.slice(0, 3).map((reminder) => {
                    ...
                    return (
                      <div key={reminder.id} className="bg-white dark:bg-slate-900 p-2.5 md:p-3 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800" style={{ borderLeft: `3px solid ${color}` }}>
