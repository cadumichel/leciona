    ...
    <div className="space-y-6 md:space-y-8 pb-20">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[32px] md:rounded-[40px] p-6 md:p-8 w-full max-w-2xl shadow-2xl animate-in zoom-in-95 max-h-[90vh] overflow-y-auto">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  >
                    ...
                    {(activeSchool?.classes || []).filter(c => typeof c === 'string' || !c.deleted).map(c => {
            ...
            <div className="mt-8 flex gap-4">
              <button
                ...
                className="flex-none py-4 px-6 rounded-xl md:rounded-2xl font-black uppercase text-[9px] md:text-[10px] tracking-widest bg-slate-50 text-blue-600 hover:bg-blue-50 transition-colors border-2 border-slate-100 hover:border-blue-100"
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-[32px] w-full max-w-lg shadow-2xl p-6 flex flex-col max-h-[90vh]">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  {(copyTargetSchool?.classes || []).filter(c => typeof c === 'string' || !c.deleted).map(c => {
                    const cId = typeof c === 'string' ? c : c.id;
                    const cName = typeof c === 'string' ? c : c.name;
                    return <option key={cId} value={cId}>{cName}</option>;
                  })}
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl px-4 py-3 text-xs font-bold outline-none dark:text-white disabled:opacity-50"
                >
                  <option value="">
            ...
            <div className="mt-6 flex gap-4">
              <button onClick={() => setCopyData(prev => ({ ...prev, isOpen: false }))} className="flex-1 py-3 bg-slate-100 text-slate-500 rounded-xl font-black uppercase text-[9px] tracking-widest hover:bg-slate-200 transition-all">Cancelar</button>
              <button
                onClick={handleCopyAssessment}
                disabled={!copyData.targetSlotId || !newEvent.title}
                className="flex-1 py-3 bg-blue-600 text-white rounded-xl font-black uppercase text-[9px] tracking-widest shadow-lg shadow-blue-200 hover:bg-blue-700 disabled:opacity-50 disabled:shadow-none transition-all"
              >
                Confirmar Cópia
              </button>
            </div>
      ...
      <div className="flex flex-col gap-2 md:gap-3">
        {data.events.filter(e => e.type === 'test' || e.type === 'work').sort((a, b) => a.date.localeCompare(b.date)).map(event => {
          ...
          return (
            <div
              ...
              className="bg-white dark:bg-slate-900 p-2 md:p-3 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm relative group hover:shadow-md transition-all duration-300 cursor-pointer hover:border-blue-200 dark:hover:border-blue-800 flex items-center gap-3 md:gap-4"
              style={{ borderColor: color + '30', backgroundColor: color + '05' }}
            >
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl flex items-center justify-center border-b-2 shrink-0" style={{ backgroundColor: color + '15', color: color, borderColor: color + '30' }}>
        ...
        {data.events.filter(e => e.type === 'test' || e.type === 'work').length === 0 && (
          <div className="py-16 md:py-20 bg-white dark:bg-slate-900 rounded-[32px] border-2 border-dashed border-slate-100 dark:border-slate-800 flex flex-col items-center justify-center gap-4 text-center">
