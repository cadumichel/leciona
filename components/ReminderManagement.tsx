    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredReminders.map(r => {
          ...
          return (
            <div key={r.id} className="bg-white rounded-[32px] border border-slate-100 shadow-sm p-6 flex flex-col relative group hover:shadow-xl transition-all duration-300 animate-in fade-in" style={iconColor ? { borderLeft: `3px solid ${iconColor}` } : {}}>
              <div className="flex justify-between items-start mb-4">
                <div className={`p-2 rounded-xl ${!iconColor ? `${cat.bg} ${cat.color}` : ''}`} style={iconStyle}>
                  <cat.icon size={20} />
                </div>
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="bg-red-50 hover:bg-red-500 text-red-500 hover:text-white transition-all p-2.5 rounded-xl hover:scale-110 shadow-sm hover:shadow-md"
        ...
        {filteredReminders.length === 0 && (
          <div className="col-span-full py-20 bg-white rounded-[48px] border-2 border-dashed border-slate-100 flex flex-col items-center justify-center gap-4 text-center">
            ...
            <div>
              <p className="text-slate-500 font-black uppercase text-xs tracking-widest">Nenhum lembrete encontrado</p>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">Assunto / Título</label>
                  <input
                    type="text"
                    value={form.title}
                ...
                {/* School Selector */}
                <div className="col-span-2">
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">Vincular a Escola (Opcional)</label>
                  <select
                    value={form.schoolId}
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">Conteúdo da Nota</label>
                <textarea
                  rows={4}
                  value={form.content}
