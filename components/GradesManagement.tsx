...
export default function GradesManagement({ data, onUpdateData }: GradesManagementProps) {
  ...
  const [newColumnForm, setNewColumnForm] = useState({ title: '', weight: 1, date: new Date().toISOString().split('T')[0] });
  ...
  useEffect(() => {
    if (selectedSchoolId) {
      ...
      if (school && school.classes.length > 0 && !school.classes.includes(selectedClassId)) {
        setSelectedClassId(school.classes[0]);
      }
    }
  }, [selectedSchoolId, data.schools]);
  ...
  const handleAddColumn = () => {
    ...
    setNewColumnForm({ title: '', weight: 1, date: new Date().toISOString().split('T')[0] });
            ...
            <div>
               ...
               <div className="relative">
                  <select value={selectedClassId} onChange={e => setSelectedClassId(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl px-4 py-3 text-xs font-bold dark:text-white appearance-none cursor-pointer" disabled={!selectedSchoolId}>
                     ...
                     {data.schools.find(s => s.id === selectedSchoolId)?.classes.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                  <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" size={14} />
               </div>
            </div>
                     ...
                     {columns.map((col, idx) => (
                        <th key={col.id} className="p-4 min-w-[140px] text-[10px] font-black text-slate-500 uppercase tracking-widest text-center relative group">
                           <div className="flex flex-col items-center gap-1">
                              ...
                              {/* DATA VISÍVEL */}
                              <div className="flex items-center gap-1 text-[9px] text-slate-400 font-bold bg-slate-100 dark:bg-slate-700/50 px-2 py-0.5 rounded-lg mt-0.5">
