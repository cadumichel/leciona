...
const Dashboard: React.FC<DashboardProps> = ({ data, onUpdateData, onNavigateToLesson, onNavigateToReminders, onNavigateToPendencies, onNavigateToAssessments, onNavigateToAgenda }) => {
  ...
  const isLessonBlocked = (dateStr: string, schoolId: string, shiftId?: string, classId?: string) => {
    // Find calendar that covers this date
    const calendars = data.calendars.filter(c => c.schoolId === schoolId);
    const calendar = calendars.find(c => dateStr >= c.start && dateStr <= c.end);
    ...
    // If we have calendars but this date is not in any of them, it's a break/vacation
    if (!calendar && calendars.length > 0) return true;
  ...
  const pendingCount = useMemo(() => {
    ...
    for (let i = 0; i < 300; i++) {
      ...
      data.schedules.filter(s => Number(s.dayOfWeek) === dayOfWeek && s.classId !== 'window').forEach(s => {
        ...
        // Calendar check is now implicitly handled by isLessonBlocked
  ...
  const renderDashboardCalendar = () => {
    ...
    return (
      <div className="bg-white dark:bg-slate-900 p-4 md:p-6 rounded-[24px] md:rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-sm">
        <div className="flex justify-between items-center mb-4 md:mb-6">
          <div className="flex items-center gap-2">
            <CalendarIcon className="text-primary" size={16} />
            <h4 className="text-[10px] md:text-xs font-black text-slate-800 dark:text-white uppercase tracking-widest">{dashMonth.toLocaleString('pt-BR', { month: 'long', year: 'numeric' })}</h4>
          </div>
          <div className="flex gap-1">
            <button onClick={() => setDashMonth(new Date(year, month - 1))} className="p-1 md:p-1.5 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg md:rounded-xl transition-all border border-slate-100 dark:border-slate-800"><ChevronLeft size={14} /></button>
            <button onClick={() => setDashMonth(new Date(year, month + 1))} className="p-1 md:p-1.5 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg md:rounded-xl transition-all border border-slate-100 dark:border-slate-800"><ChevronRight size={14} /></button>
          </div>
        </div>
        <div className="grid grid-cols-7 gap-1">
          {['D', 'S', 'T', 'Q', 'Q', 'S', 'S'].map((d, i) => (
            <div key={`${d}-${i}`} className="text-center text-[7px] md:text-[8px] font-black text-slate-400 py-1 uppercase tracking-tighter">{d}</div>
          ))}
          {days.map((d, i) => {
            ...
            return (
              <div key={i} className={`aspect-square rounded-lg md:rounded-xl flex flex-col items-center justify-center relative border transition-all ${cellBg}`}>
    ...
    <div className="space-y-4 md:space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-3 md:gap-4">
        <div>
          ...
          <p className="text-[9px] md:text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mt-1 md:mt-2">
            {new Date().toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' })}
          </p>
        </div>
        <div className="flex items-center gap-2 md:gap-3 bg-white dark:bg-slate-900 px-3 py-1.5 md:px-4 md:py-2 rounded-xl md:rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm shrink-0 w-fit">
          <Sparkles className="text-primary animate-pulse w-3 h-3 md:w-4 md:h-4" />
          <span className="text-[8px] md:text-[9px] font-black uppercase tracking-widest text-slate-500">Inspirado para hoje</span>
        </div>
      </div>

      {pendingCount > 0 && !data.settings.hideUnregisteredClassesOnDashboard && (
        <button onClick={onNavigateToPendencies} className="w-full flex items-center justify-between p-3 md:p-4 rounded-[20px] md:rounded-[24px] bg-red-600 text-white shadow-lg shadow-red-200 dark:shadow-red-900/20 hover:scale-[1.01] transition-transform animate-in slide-in-from-top-4">
      ...
      <div className="grid lg:grid-cols-3 gap-4 md:gap-6">
        <div className="lg:col-span-2 space-y-4 md:space-y-6">
          <div className="bg-white dark:bg-slate-900 p-4 md:p-6 rounded-[24px] md:rounded-[32px] shadow-sm border border-slate-100 dark:border-slate-800">
            <h3 className="text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-4 md:mb-6 flex items-center gap-2"><Clock className="text-primary w-3 h-3 md:w-[14px] md:h-[14px]" /> Atividade Agora</h3>
            {activityInfo ? (
              <div className="space-y-3 md:space-y-4">
                {/* CARTÃO PRINCIPAL DA AULA ATUAL */}
                <button onClick={() => onNavigateToLesson(activityInfo.current.schedule, todayDateStr)} className="w-full text-left p-4 md:p-6 rounded-[24px] md:rounded-[32px] transition-all hover:scale-[1.01] shadow-xl border-2 relative overflow-hidden group" style={{ backgroundColor: (activityInfo.current.type === 'school' ? activityInfo.current.school?.color : activityInfo.current.student?.color) + '15', borderColor: (activityInfo.current.type === 'school' ? activityInfo.current.school?.color : activityInfo.current.student?.color) + '40' }}>
                  <div className="absolute top-0 right-0 bg-white/50 dark:bg-black/20 px-3 py-1.5 md:px-4 md:py-2 rounded-bl-xl md:rounded-bl-2xl backdrop-blur-sm border-l border-b border-white/20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                  <div className="p-3 md:p-4 rounded-xl md:rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 flex flex-col justify-center">
                    <div className="flex items-center gap-2 mb-1 md:mb-2 text-slate-400">
                      <History size={10} className="md:w-3 md:h-3" />
                      <span className="text-[7px] md:text-[8px] font-black uppercase tracking-widest">Aula Anterior</span>
                    </div>
                  ...
                  <div className="p-3 md:p-4 rounded-xl md:rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 flex flex-col justify-center">
                    <div className="flex items-center gap-2 mb-1 md:mb-2 text-slate-400">
                      <ArrowRight size={10} className="md:w-3 md:h-3" />
                      <span className="text-[7px] md:text-[8px] font-black uppercase tracking-widest">Próxima Aula</span>
                    </div>
                    {activityInfo.isLast ? (
                      <div className="flex items-center gap-2 text-green-600 dark:text-green-400">
                        <CheckCircle2 size={12} className="md:w-[14px] md:h-[14px]" />
                        <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest">Fim do expediente! 🎉</span>
                      </div>
                    ) : (
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
              ...
              <div className="bg-slate-50 dark:bg-slate-800/30 p-4 md:p-6 rounded-[24px] md:rounded-[32px] text-center border-4 border-dashed border-slate-200 dark:border-slate-800">
                <Coffee className="mx-auto text-slate-400 mb-2 md:mb-3" size={24} />
                <p className="text-slate-500 font-black uppercase text-[9px] md:text-[10px] tracking-widest">Pausa na rotina</p>
                <p className="text-slate-500 font-bold text-[8px] md:text-[9px] uppercase mt-1">Nenhum horário em andamento agora</p>

                {nextGlobalActivity && (
                  <div className="mt-4 md:mt-5 pt-4 md:pt-5 border-t border-slate-200 dark:border-slate-700">
                    <p className="text-[8px] md:text-[9px] font-black text-blue-500 uppercase tracking-widest mb-2 md:mb-3 flex items-center justify-center gap-2">
                    ...
                    <div className="flex flex-col items-center gap-2">
                      <div className="inline-flex items-center gap-3 px-3 py-1.5 md:px-4 md:py-2 bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
                        <span className="text-[10px] md:text-xs font-black text-slate-700 dark:text-slate-200">{nextGlobalActivity.startTime}</span>
                        <div className="w-1 h-1 bg-slate-300 rounded-full" />
                        <div className="text-left">
                          <p className="text-[9px] md:text-[10px] font-black uppercase text-primary">
          ...
          <div className="grid sm:grid-cols-2 gap-4 md:gap-6">
            <div className="bg-white dark:bg-slate-900 p-4 md:p-6 rounded-[24px] md:rounded-[32px] shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col">
              ...
              <div className="space-y-2 md:space-y-3 flex-1">
                {upcomingAssessments.length > 0 ? upcomingAssessments.map((event, idx) => {
                  ...
                  return (
                    <div key={idx} onClick={onNavigateToAssessments} className="flex items-center gap-3 p-2.5 md:p-3 bg-slate-50 dark:bg-slate-800/40 rounded-[20px] md:rounded-[24px] border border-slate-100 dark:border-slate-800 cursor-pointer hover:scale-[1.02] transition-transform" style={{ borderColor: color + '30', backgroundColor: color + '05' }}>
                  ...
                  <div className="py-6 md:py-8 text-center flex flex-col items-center justify-center h-full"><p className="text-[8px] md:text-[9px] font-black uppercase tracking-widest text-slate-500">Sem avaliações próximas</p></div>
                )}
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 p-4 md:p-6 rounded-[24px] md:rounded-[32px] shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col">
              ...
              <div className="space-y-2 md:space-y-3 flex-1">
                {upcomingEvents.length > 0 ? upcomingEvents.map((event, idx) => {
                  ...
                  return (
                    <div key={idx} onClick={onNavigateToAgenda} className="flex items-center gap-3 p-2.5 md:p-3 bg-slate-50 dark:bg-slate-800/40 rounded-[20px] md:rounded-[24px] border border-slate-100 dark:border-slate-800 cursor-pointer hover:scale-[1.02] transition-transform" style={{ borderColor: color + '30', backgroundColor: color + '05' }}>
                      <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl flex flex-col items-center justify-center shrink-0 border-b-2" style={{ backgroundColor: color + '15', color: color, borderColor: color + '30' }}>
                        <span className="text-[7px] md:text-[8px] font-black uppercase leading-none mb-0.5">{getShortWeekDay(event.date)}</span>
                        <span className="text-[10px] md:text-xs font-black leading-none">{getDayMonth(event.date)}</span>
                      </div>
                  ...
                  <div className="py-6 md:py-8 text-center flex flex-col items-center justify-center h-full"><p className="text-[8px] md:text-[9px] font-black uppercase tracking-widest text-slate-500">Sem eventos próximos</p></div>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="space-y-4 md:space-y-6">
          ...
          <div className="bg-amber-50 dark:bg-amber-900/10 p-4 md:p-6 rounded-[24px] md:rounded-[32px] border border-amber-100 dark:border-amber-900/20">
            <div onClick={onNavigateToReminders} className="flex items-center justify-between mb-3 md:mb-4 cursor-pointer group">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-[9px] md:text-[10px] font-black uppercase text-amber-600 tracking-widest">Notas & Lembretes</h4>
              </div>
            ...
            {data.reminders.length > 0 ? (
              <div className="space-y-2">
                {data.reminders.slice(0, 3).map((reminder) => {
                  ...
                  return (
                    <div key={reminder.id} onClick={onNavigateToReminders} className="bg-white dark:bg-slate-900 p-2.5 md:p-3 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 cursor-pointer hover:translate-x-1 transition-transform" style={{ borderLeft: `3px solid ${color}` }}>
