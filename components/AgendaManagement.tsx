...
const AgendaManagement: React.FC<AgendaManagementProps> = ({ data, onUpdateData }) => {
  ...
  return (
    <div className="space-y-6 md:space-y-8 pb-20">
      <div className="bg-white dark:bg-slate-900 p-4 md:p-6 rounded-[24px] md:rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col md:flex-row md:items-center gap-4 transition-all">
        <div className="flex-1">
          <label className="block text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 flex items-center gap-1"><SchoolIcon size={12}/> Visualizar Agenda de:</label>
          <div className="relative">
            <select value={activeSchoolId} onChange={e => setActiveSchoolId(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl md:rounded-2xl px-4 md:px-5 py-3 font-bold text-slate-700 dark:text-white outline-none focus:ring-2 focus:ring-primary cursor-pointer appearance-none text-sm">
      ...
      <div className="bg-white dark:bg-slate-900 p-5 md:p-8 rounded-[32px] md:rounded-[48px] border border-slate-100 dark:border-slate-800 shadow-sm space-y-4 md:space-y-6">
          <div className="flex justify-between items-center">
            ...
            <button onClick={() => setIsAddingEvent(true)} className="bg-orange-500 text-white px-4 md:px-5 py-2 md:py-2.5 rounded-xl md:rounded-2xl font-black uppercase text-[9px] md:text-[10px] tracking-widest shadow-lg shadow-orange-100 hover:brightness-110 transition-all flex items-center gap-2">
              <Plus size={14} className="md:w-[16px] md:h-[16px]" /> Novo Registro
            </button>
          </div>
          <div className="bg-slate-50 dark:bg-slate-800/50 p-4 md:p-6 rounded-[24px] md:rounded-[40px] border border-slate-100 dark:border-slate-800">
                  ...
                  <div key={event.id} className="bg-white dark:bg-slate-900 p-4 md:p-5 rounded-[20px] md:rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-sm transition-all flex items-center gap-3 md:gap-4 hover:shadow-md" style={{ borderColor: `${eventColor}40` }}>
                      ...
                      <div className="flex-1 overflow-hidden">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                            <p className="text-[8px] md:text-[9px] text-slate-400 font-bold uppercase truncate">{event.type === 'festivity' ? 'Evento' : event.type}</p>
                            {activeSchoolId === 'all' && (
                               <span className="text-[7px] md:text-[8px] font-black bg-slate-100 dark:bg-slate-800 text-slate-500 px-2 py-0.5 rounded-lg uppercase truncate max-w-[80px] md:max-w-[100px]">
                                  {school?.name}
                               </span>
                            )}
                            <span className="text-[7px] md:text-[8px] font-black bg-orange-50 dark:bg-orange-900/30 text-orange-600 px-2 py-0.5 rounded-lg uppercase">
                               {getEventScope(event, school)}
                            </span>
                            {event.blocksClasses && (
                              <span className="text-[7px] md:text-[8px] font-black bg-red-100 dark:bg-red-900/30 text-red-600 px-2 py-0.5 rounded-lg uppercase">
                                Cancela Aulas
                              </span>
                            )}
                        </div>
                      </div>
                      <button onClick={() => onUpdateData({ events: data.events.filter(e => e.id !== event.id) })} className="text-slate-200 hover:text-red-500 p-1"><Trash2 size={16} /></button>
                  </div>
                );
              }) : (
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-[32px] md:rounded-[40px] p-6 md:p-8 w-full max-w-xl shadow-2xl animate-in zoom-in-95 max-h-[90vh] overflow-y-auto">
             ...
             <div className="space-y-4 md:space-y-6">
                ...
                <div>
                  <label className="block text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Instituição</label>
                  <select value={eventForm.schoolId} onChange={e => setEventForm({...eventForm, schoolId: e.target.value, shiftId: '', classId: ''})} className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl md:rounded-2xl px-4 md:px-5 py-3 font-bold dark:text-white outline-none focus:ring-2 focus:ring-primary text-sm">
                    {data.schools.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                  </select>
                </div>

                <div>
                  <label className="block text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Título do Evento</label>
                  <input type="text" value={eventForm.title} onChange={e => setEventForm({...eventForm, title: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl md:rounded-2xl px-4 md:px-5 py-3 font-bold dark:text-white text-sm" placeholder="Ex: Conselho de Classe" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                   <div>
                     <label className="block text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Tipo</label>
                     <select value={eventForm.type} onChange={e => setEventForm({...eventForm, type: e.target.value as any})} className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl md:rounded-2xl px-4 md:px-5 py-3 font-bold dark:text-white text-sm">
                   ...
                   <div>
                     <label className="block text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Data</label>
                     <input type="date" value={eventForm.date} onChange={e => setEventForm({...eventForm, date: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl md:rounded-2xl px-4 md:px-5 py-3 font-bold dark:text-white text-sm" />
                   </div>
                </div>

                <div className="bg-orange-50 dark:bg-orange-900/10 p-4 md:p-5 rounded-3xl border border-orange-100 dark:border-orange-900/30">
                   ...
                   {eventForm.blocksClasses && (
                      <div className="space-y-4 animate-in slide-in-from-top-2">
                         {conflictWarning && (
                            <div className="bg-white dark:bg-slate-900/80 p-3 rounded-2xl border-l-4 border-red-500 flex items-start gap-3 shadow-sm">
                            ...
                            <div className="grid grid-cols-3 gap-2">
                               {([
                                 ...
                                 <button 
                                  ...
                                  className={`flex flex-col items-center justify-center p-2 rounded-xl border-2 transition-all ${eventForm.cancelScope === scope.id ? 'bg-white dark:bg-slate-800 border-orange-500 text-orange-600 shadow-sm' : 'bg-orange-100/30 dark:bg-orange-900/20 border-transparent text-orange-400'}`}
                                 >
                         ...
                         {eventForm.cancelScope === 'shift' && (
                            <div className="animate-in fade-in zoom-in-95">
                               ...
                               <select value={eventForm.shiftId} onChange={e => setEventForm({...eventForm, shiftId: e.target.value})} className="w-full bg-white dark:bg-slate-800 border-none rounded-xl px-3 py-2 text-xs font-bold text-orange-600 dark:text-orange-400">
                         ...
                         {eventForm.cancelScope === 'class' && (
                            <div className="animate-in fade-in zoom-in-95">
                               ...
                               <select value={eventForm.classId} onChange={e => setEventForm({...eventForm, classId: e.target.value})} className="w-full bg-white dark:bg-slate-800 border-none rounded-xl px-3 py-2 text-xs font-bold text-orange-600 dark:text-orange-400">
                                  <option value="">Escolha...</option>
                ...
                <div>
                   <label className="block text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Descrição (Opcional)</label>
                   <textarea value={eventForm.description} onChange={e => setEventForm({...eventForm,description: e.target.value})} rows={3} className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl md:rounded-2xl px-5 py-3 font-bold dark:text-white text-sm" placeholder="Detalhes extras..." />
                </div>
             </div>
             <div className="mt-8 mb-4 flex gap-4"><button onClick={() => setIsAddingEvent(false)} className="flex-1 py-4 font-black text-slate-400 uppercase text-[9px] md:text-[10px]">Cancelar</button><button onClick={handleSaveEvent} className="flex-1 py-4 bg-orange-500 text-white rounded-xl md:rounded-2xl font-black uppercase text-[9px] md:text-[10px] shadow-xl shadow-orange-100">Salvar Registro</button></div>
          </div>
        </div>
      )}
    </div>
